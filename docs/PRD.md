...
ExamGroup (unused for live class. Zoom breakouts stay in Zoom. Table kept for old rows.)
  ├── id (uuid)
...
GroupExamSubmission (one student's answers)
  ...
  ├── exam_group_id (uuid, nullable, FK → ExamGroup — unused for live class; null for new rows)
  ├── course_session_id (uuid, FK → CourseSession)
...
| 49b. Exam group formation (teacher) | Teacher creates ExamGroups of 2-3 students who have opened the exam (`first_opened_at`). No designated writer. Each grouped student writes their own answers. Live-updating picker. | Create groups from opened students only, verify late openers appear, verify ungrouped live students cannot type | **DROPPED 2026-09-24.** Kyle splits Zoom breakouts by hand. Session page has no group picker. Live students type after Iniciar without an app group. `exam_groups` table remains for old rows. |
...
| 49e. Post-class review (student) | Attended: full keys (teacher accepted, catalog fallback) + score. Absentee: 45-min makeup, Entregar per task then keys for that task, score after all three. | Terminar clase shows keys for an attended account; a not-attended account must Entregar Tarea 1 before those keys appear | **BUILT 2026-09-23.** Empty catalog answers skip marks and drop out of the score total (smart paste, 2026-09-24). |
| 49f. Smart paste exam + post-class answers | ExamEditor and Nueva clase accept Kyle's natural paste (English-only vocab, `(español)` Task 1 slots, bare Task 2/3 lines). Answers optional at create. Serialize round-trips to bare format until answers exist. Intermediate Order shuffles only when stored positions are 1,2,3…. Contenido: live student preview, collapsible Respuestas, copy live `exam_class_answers` from a session of this prompt. No schema migration. Not PRD slice 50 (that row stays reverse translation). | Paste Supersticiones-style lines, save, reload, see the same bare text. After class, copy type-check answers into the catalog, save, absentee makeup gets green/red. | **BUILT 2026-09-24.** Plan review: full spec; identity-only scramble. |
...
| 66. Catalog CRUD v1 (delete + simple create) | Delete any catalog type from `/teacher/content` when no `course_sessions` row references it (archived months count). Children deleted parent-last. Confirm lists non-zero counts. Consumer/self-study rows warn but do not block. Delete gated to `CATALOG_ADMIN_EMAILS` (no admin role). Create writing/exam/presentation/conversation via `/teacher/content/{type}/new` then the Slice 55 editor. Story kinds are not creatable here. Nueva clase writing/exam/conversation: Nueva lección (existing compose) or Usar una anterior (attach the existing catalog row, same as stories). Presentation still picks existing. No schema migration. **BUILT 937feae 2026-09-21.** Deviations vs original prompt: Nueva clase compose-or-copy added in this slice; delete env-gated rather than any-teacher; no `dictation_prompts` table (drill = dictado). **2026-09-24:** Usar una anterior stopped cloning; it assigns the chosen row. Student data stays on the session. Editing Contenido still changes every class that uses that handout. | Create a throwaway writing/exam/presentation/conversation, land in the editor, see it on the index. Delete unreferenced throwaways. Assigned content is blocked with the Spanish class-count message. |
...
| 67b. Lesson header view toggle | On a session-backed lesson, teachers get **Vista de estudiante** left of Inicio. Click sets `pk_student_preview` from the course level and redirects to the same URL. Label becomes **Vista de profe** (clears cookie, stays on the lesson). Inicio stays `/teacher` during preview. Rail whole-app path unchanged. Open `/lesson/slug` without a session: no toggle. No schema. **BUILT c148249 2026-09-22.** | Open a class link as teacher → Vista de estudiante → student chrome, same URL, Inicio → `/teacher`. Vista de profe restores teacher pills. Students never see the control. |
...
2. Students open the exam link (that marks attendance). Zoom discussion groups stay in Zoom. The app does not assign groups.
...
16. **Group exam: individual answers.** Zoom breakouts stay in Zoom. The teacher class page does not split students into app groups. Every student writes their own exam. Live check is the teacher typing accepted answers and tapping check per item. Score is practice math (correct/total), not a CEFR or official grade.
