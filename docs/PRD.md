...
CourseSession (one class meeting, one catalog activity assigned)
  ├── id (uuid)
  ├── course_id (uuid, FK → Course)
  ├── session_type (enum: "story" | "writing" | "exam" | "video_summary" | "presentation" | "conversation" | "pronunciation" | "dialogue" | "movie_talk" | "song" — presentation uses presentation_prompt_id; conversation uses conversation_prompt_id (Slice 61); pronunciation remains live-only, no prompt FK, no story_id (Slice 65: Nueva clase can create it); dialogue/movie_talk/song point at story_id with the matching Story.kind since Slice 62)
  ├── story_id (uuid, FK → Story, nullable — required when session_type is "story")
  ...
  └── created_at (timestamp)
  CHECK: story sessions have story_id and no writing_prompt_id/exam_prompt_id/presentation_prompt_id; writing sessions have writing_prompt_id and no story_id/exam_prompt_id/presentation_prompt_id; exam sessions have exam_prompt_id and no story_id/writing_prompt_id/presentation_prompt_id; video_summary sessions have story_id (where Story.kind = "video_summary") and no writing_prompt_id/exam_prompt_id/presentation_prompt_id; presentation sessions have presentation_prompt_id and no story_id/writing_prompt_id/exam_prompt_id; dialogue sessions (Slice 62) have story_id (where Story.kind = "dialogue") and no writing_prompt_id/exam_prompt_id/presentation_prompt_id/conversation_prompt_id; song sessions (Slice 63) have story_id (where Story.kind = "song") and no writing_prompt_id/exam_prompt_id/presentation_prompt_id/conversation_prompt_id — song sessions have existed since Slice 62 with their own session_type; Slice 63 adds the lesson_step pacing fields on them. One session, one activity. (Schema-level: `course_sessions_session_type_check` and `course_sessions_activity_check` both gain a dialogue branch.)
...
>
> **Timer behavior:** Pre-intermediate: 10-min countdown, auto-submit at zero, text input locks. Intermediate: 20-min countdown, visual alert at zero but input stays open (Kyle allows continued writing). Submit button is always available for both levels.
>
...
| 22b. Student writing page + timer | Student clicks session link → sees writing page: prompt at top, optional structure lesson/rubric/example (intermediate), text input, timer, submit button, live word count. Timer starts when teacher clicks "Iniciar" (Supabase Realtime sync to all connected students). Pre-intermediate: 10-min countdown, auto-submit at zero, input locks. Intermediate: 20-min countdown, visual alert at zero, input stays open. Word count displayed live for both levels. WPM displayed for pre-intermediate only. 5-minute warning shown. | Open session link as student, write, verify timer counts down, verify word count updates, submit |
