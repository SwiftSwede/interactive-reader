...
4. **Completa la canción** (hidden when `lyric_blanks` is empty) — **the live worksheet, the centerpiece.** Full lyrics visible at once (LyricTraining-style, NOT line-by-line), with the 8-10 numbered blanks rendered INLINE at their positions in the lyric lines. Repeated words reuse the same blank number (deck convention — the existing data already encodes this via blank ids). Sticky mini YouTube player (sticky elements may use subtle shadow per DESIGN.md; reuse the StoryAudioPlayer sticky pattern for placement). In live classroom mode the mini player follows the teacher sync when students tap follow.
   ...
   - **Autosave:** every keystroke, debounced ~1.5s and on blur, upserts `song_lyric_attempts` rows (one per blank). A refresh mid-exercise never loses work. This is the load-bearing exception — see Kyle's Rules below. A multi-word answer (`pull up`) is still one blank id: one number, one underline per word, typed parts joined for save and score.
...
6. **Truquitos y karaoke** (hidden when `youtube_url` is null) — lyrics with a **line-swap toggle** (one switch, top of the step: "Truquitos" on/off): each lyric line swaps between clean text and its connected-speech IPA from `lyrics_ipa` (keyed by `line_index`; lines without an IPA entry stay clean). Toggle is line-swap, NOT hover tooltips (students can't hover while singing). Karaoke: current line highlighted driven by `line_timestamps` against the YouTube embed playhead; the highlight bar slides to the next line in 200ms (native CSS). The page does not scroll on every line. When the highlight would tuck under the sticky player or the header, lyrics recenter in the visible well. After first play, reuse the story sticky transport for play/pause / ±10s / speed / seek (live students excluded). Graceful degradation — no timestamps means no highlight, the step still shows the toggle. **Rewind:** review/consumer ONLY: one button to the left of the current highlighted line; tap replays that line from its start. Tapping any timestamped lyric line jumps the embed to that line's start and plays. Missing timestamps hide rewind and line-tap.
...
4. `src/components/music/SongBlanksWorksheet.tsx` — the centerpiece (spec in "What to build" (b) step 4). Props: story (lyrics body_text, lyric_blanks, youtube_url), session context (mode, answers_revealed, video sync), initial attempts (hydrated from `song_lyric_attempts` on load — a refresh resumes exactly where the student was), callbacks via server actions. Inline blanks: render lyric lines with numbered inputs at blank positions (blank ids match the existing `lyric_blanks` array; a multi-word answer is one numbered blank with one underline per word).
...
6. `src/components/music/SongTruquitosKaraoke.tsx` — line-swap toggle, karaoke highlight from `line_timestamps` (recenter only when the line would tuck under the sticky player or header), story sticky transport after first play, rewind button that follows the highlight on the left, tap any timestamped line to jump there (review/consumer only), YouTube embed (teacher-synced in classroom mode, normal controls otherwise).
