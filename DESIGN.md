...
- **Este mes (default):** current calendar month. One card per group (course): group name, level label, day/time pattern, theme if set, readiness summary ("3/8 clases listas"), next class with countdown. If a group has a class today (join window from T-10 through session end, or countdown earlier that day), a class-day card (same countdown / join / done phases as student Inicio) shows **Abrir la clase** (teacher session URL) and **Entrar a Zoom** when `zoom_url` is set. Same card in the right panel when that group is selected, on Grupos, and on the group page. Clicking a group expands/drills into its 8-class strip. If no course exists for the current month: empty state "Próximo mes en preparación" with a "Nuevo mes" button (generator ships in a later slice; until then the button routes to the existing create-course form).
