...
- Task 2: Order (letters A, B, C label scrambled sentences; numbers 1..N in the blank) or Correct (correct/incorrect + edit). Intermediate Order: if stored positions are already 1,2,3… (bare paste in correct order), shuffle display with a stable seed from the prompt id so every phone sees the same scramble. Numbered paste (`3 | Last`) keeps stored order. Teacher live sees the same list as students. TeacherPeek is still letter → correct number.
- Task 3: Translate. Textareas per Spanish sentence.
- Teacher live: blank fields stored on `exam_class_answers`. One row: Lucide Plus (no box) left of the field, 44px check on the right. Extra variants stack under the field. Collapsible catalog peek. Mostrar puntaje when every item is already checked.
- Puntaje: `Tu puntaje`, fraction and whole percent. Spanish around the numbers. No letter grade, no CEFR. Items with no catalog or live accepted answers are left out of the fraction.

**After class:**
- Attended: keys when answers exist (teacher accepted, then catalog). No green/red if the catalog still has no English.
...
- Step 3 Traducción: opens for students when the writing timer hits zero. No teacher Continuar / Empezar traducción button. Spanish paragraphs as Lora `story-body` (no word tooltips). English as Roboto Flex `story-english` (same size, sans). Teacher textarea uses `story-english` with terracotta 2px border, including before class for prep. Listo (moss `--success`) stays while teaching mode is on, including after the scheduled 90 minutes. Students see live English via Realtime. If a paragraph has no English yet, show nothing. Never show "El profe está traduciendo..." Teacher-only collapsible English original during class. After Terminar clase, students also get "Ver inglés original" plus the live class translation.
- Teaching notes: yellow highlight (`teaching-note-mark`, same yellow as karaoke). The mark stays inline and does not expand the line. Tap opens a small lightbox (`teaching-note-modal`, 24px radius, dimmed backdrop) with the note. Teacher selects text during live class; the compose form is the same lightbox. Teacher lightbox includes Borrar. Notes belong to this class session only and to one side (Spanish or English). Teacher English notes show as yellow word chips under the box, not a second copy of the paragraph. Teacher taps Terminar clase at the bottom of Traducción (and the last step of other lesson types) to turn on review mode.
...
- Completa la canción: compact player under the title and instructions, then lyrics. Lyric block is centered; lines are `text-align: left`. Lyrics use `text-story-body` (Lora 18px / 32px). Blanks are underlines in the line (same size as the lyrics, caret on focus), not tall input boxes. Empty blanks are 3 characters wide and grow with the typed word. A multi-word answer (`pull up`) is one numbered blank with one underline per word; typed parts join as one answer. After Entregar, each underline hugs that word. Each blank is numbered in a 20px terracotta (`--accent`) circle with white centered `label-sm` text. Wrong answers: student words, then the correct phrase in moss `label-sm`, then the X. Live class: Entregar only, no marks; teacher typing in a blank replaces that blank on student phones (student submissions stay saved). Review and self-study: one Entregar tap shows marks immediately.
...
- Truquitos y karaoke: same lyric cluster (title left-aligned with the lines, stanza gaps). Line-swap toggle (starts off). Karaoke highlight from `line_timestamps` vs embed playhead; the highlight bar slides to the next line in 200ms (native CSS, no extra library; `prefers-reduced-motion` jumps). The page does not scroll on every line. When the highlight would tuck under the sticky player (or the header), lyrics recenter so that line sits in the middle of the visible well. After first play, the story sticky transport (play/pause, ±10s, speed, seek) controls the YouTube embed so pause stays on screen. Live students do not get the sticky bar (teacher lock). Review/consumer: one rewind button to the left of the current highlighted line; tapping any timestamped lyric line jumps to that line's start and plays.
