...
**TIMESTAMP-OWNERSHIP RULE (tap-align data):** `line_timestamps` is TAP-ALIGN-OWNED. `seed-music.ts` omits the column from its upsert when the song entry defines none — a bare `?? null` in that upsert wiped Kyle's tap-aligned karaoke JSON on 2026-09-16 (re-seeds during the blanks/meaning fixes). The seed now logs "preserving N existing line_timestamps" when the DB carries them, and warns on any DB-vs-SONGS drift (body_text, lyric_blanks, artist_bio, song_meaning, youtube_url, lyrics_ipa) before overwriting — so Supabase Table Editor edits are never silently reverted. Rule 18 in `.cursorrules` generalizes this: every seed script declares which columns it owns, and none of them may write null over teacher-entered or tool-generated data. Recovery note: tap-align sessions auto-save in the tool (localStorage, per video) — reopen `tap-align-lyrics.command` in the same browser to restore, Generar JSON, paste into `line_timestamps`.
